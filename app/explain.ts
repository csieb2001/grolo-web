...
export function explainSmart(lang: "en" | "de", d: ExplainInput, fmtW: (w: number | null | undefined) => string): string {
  ...
  const m = { mode: d.mode, out: fmtW(out), target: fmtW(target), house: fmtW(house), grid: fmtW(grid == null ? null : Math.abs(grid)), soc: pct(soc), lim: pct(lim),
              bat: fmtW(Math.abs(bat)), pv: fmtW(d.pv), setpoint: fmtW(setpoint), max: fmtW(max) };
  const tail = bat > 2 ? fill(x.charging, m) : bat < -2 ? fill(x.discharging, m) : (out > 2 ? fill(x.solarOnly, m) : "");
  if (!sh || !sh.enabled) return fill(x.off, m);
