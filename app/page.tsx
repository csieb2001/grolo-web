...
import { ForecastSection, type ForecastData } from "./forecast";
...
type Data = Partial<SunData> & {
  ...
  daily: { day: string; pv_kwh: number; out_kwh: number; acin_kwh?: number | null; direct_kwh?: number | null; charge_kwh: number; discharge_kwh: number; grid_kwh?: number | null; feedin_kwh?: number | null; house_kwh?: number | null }[];
  ...
  battery?: { wh_per_pct: number | null; pct_observed: number; kwh?: number } | null;
...
type Period = { out_kwh: number | null; acin_kwh?: number | null; grid_kwh: number | null; feedin_kwh: number | null };
...
export default function Page() {
  ...
  const direct = data?.today ? (data.today.direct_kwh ?? Math.max(data.today.pv_kwh - data.today.charge_kwh, 0)) : 0;
  const pieA = data?.today ? [{ name: t.direct, value: direct, color: "var(--house)" }, { name: t.intoBat, value: Math.max(data.today.pv_kwh - direct, 0), color: "var(--bat)" }] : [];
  const pieB = data?.today ? [{ name: t.fromPv, value: direct, color: "var(--pv)" }, { name: t.fromBat, value: Math.max(data.today.out_kwh - direct, 0), color: "var(--bat)" }] : [];
      ...
      <main>
        ...
        <section><h2>{t.now} <small className="muted" style={{ fontSize: 12, fontWeight: 400 }}>· {t.flow}</small></h2>
          <div className="nowwrap">
            ...
            <div className="tiles">
              ...
              {(l?.out_w ?? 0) < -2 ? <Tile k={t.acIn} v={fmtW(-(l?.out_w ?? 0))} color="var(--soc)" /> : <Tile k={t.out} v={fmtW(l?.out_w)} color="var(--house)" />}
        ...
        <section><h2>{t.energy}</h2>
          <div className="tiles" style={{ marginBottom: 14 }}>
            {data?.today?.house_kwh != null && <Tile k={`${t.today}: ${t.houseToday}`} v={fmtKwh(data.today.house_kwh)} color="var(--house)" s={t.houseSplit.replace("{g}", fmtKwh(data.today.grid_kwh)).replace("{n}", fmtKwh(data.today.out_kwh))} />}
            {data?.today?.house_kwh != null && data.today.house_kwh > 0 && <Tile k={`${t.today}: ${t.selfToday}`} v={`${Math.round(Math.min(100, data.today.out_kwh / data.today.house_kwh * 100))}`} unit="%" color="var(--bat)" s={t.selfHint} />}
            ...
            {(data?.today?.acin_kwh ?? 0) > 0.001 && <Tile k={`${t.today}: ${t.acInEnergy}`} v={fmtKwh(data?.today?.acin_kwh)} color="var(--soc)" />}
          ...
          <div className="charts">
            <div className="chart"><ResponsiveContainer><BarChart data={daily}><CartesianGrid stroke="#2c3235" /><XAxis dataKey="label" stroke="#8e8e8e" fontSize={11} /><YAxis stroke="#8e8e8e" fontSize={11} /><Tooltip {...tip} formatter={(v) => fmtKwh(Number(v))} /><Legend />
              <Bar dataKey="pv_kwh" name={t.pvYield} fill="#f2cc0c" /><Bar dataKey="out_kwh" name={t.toHouse} fill="#ff9830" />{daily.some(d => d.house_kwh != null) && <Bar dataKey="house_kwh" name={t.houseToday} fill="#5794f2" />}</BarChart></ResponsiveContainer></div>
        ...
        {(() => {
          ...
          <section><h2>{t.costs} <small className="muted" style={{ fontSize: 12, fontWeight: 400 }}>· {t.costsAt.replace("{p}", fmtCt(price * 100))} · {data?.tariff ? t.costsSet : t.costsDefault}</small></h2>
            <div className="tiles" style={{ marginBottom: 14 }}>
              ...
              {(p?.month.acin_kwh ?? 0) > 0.001 && <Tile k={`${t.month}: ${t.acInCost}`} v={eur(gridCost(p?.month.acin_kwh))} color="var(--soc)" s={fmtKwh(p?.month.acin_kwh)} />}
        ...
        {data?.rooms && <RoomsSection d={data.rooms} lang={lang} locale={locale} />}

        {data?.heatpump && <HeatSection d={data.heatpump} lang={lang} locale={locale} priceCt={data.tariff?.price_ct_kwh ?? 30} />}

        <ForecastSection d={data?.forecast ?? null} lang={lang} locale={locale} />
